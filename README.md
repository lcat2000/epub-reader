...
| 按鈕 | 說明 |
|------|------|
| **選擇檔案** | 從本機選取 `.epub` 檔案開啟 |
| **至 Google Drive 下載** | 開新分頁前往 Google Drive，下載 epub 後拖放至本頁 |
| **好讀下載** | 開新分頁前往 [好讀](https://www.haodoo.net/) 下載 epub |
...
| 風險 | 防護措施 |
|------|---------|
| 惡意 epub 執行 JS | DOMPurify 過濾；禁止 `<script>`、`<object>`、`<embed>`、`<iframe>`、`<base>`、`<form>`、`<noscript>` 等標籤；封鎖所有 `on*` 事件屬性及 `srcdoc`/`formaction` |
| CSS UI 干擾 / 釣魚排版 | `position:fixed/sticky` → `absolute/relative`、`z-index` 上限 100、移除 `@import` / `expression()` / `pointer-events:none`、封鎖 CSS 內外部 `url()` 參照 |
| Zip bomb / 資源耗盡 | 壓縮前 100 MB 上限、解壓後 300 MB 上限、單一檔案 20 MB 上限、zip 總檔案數 2000 上限、章節上限 1000、每章圖片上限 200 張 |
| 惡意外部連結 | http/https 連結點擊前顯示確認視窗，以 `noopener,noreferrer` 開新分頁 |
| 其他協定注入 | 封鎖 `javascript:`、`-moz-binding:` 等非 epub 協定 |
...
| Clickjacking | Vercel 部署透過 `vercel.json` 設定 `X-Frame-Options: DENY` 與 `frame-ancestors 'none'` HTTP header |

> **注意：** GitHub Pages 不支援自訂 HTTP header，故 `frame-ancestors` 防護僅在 Vercel 部署上有效。
...
手機版工具列在窄螢幕（≤480px）自動分兩行顯示，確保所有功能可操作。
